...
const DishDetails = () => {
  const { id } = useParams();
  const { selectedDish, isLoading: dishLoading, fetchDishById } = useDishStore();
  const { reviews, isLoading: reviewsLoading, fetchReviewsByDish, addReview } = useReviewStore();
  ...
  if (dishLoading || !selectedDish) return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="relative h-[400px] lg:h-[600px] rounded-[40px] overflow-hidden shadow-2xl">
           <img 
             src={selectedDish.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=1000&auto=format&fit=crop'} 
        ...
        <div className="flex flex-col justify-center space-y-8">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 text-lg font-medium text-slate-500">
               ...
               <span>par <span className="text-slate-900 font-bold">{selectedDish.user?.name || 'Notre Chef'}</span></span>
            </div>
          </div>
          ...
          <p className="text-xl text-slate-500 leading-relaxed font-medium">
            {selectedDish.description || "Une expérience gustative unique préparée avec les meilleurs ingrédients de saison pour éveiller vos papilles."}
          </p>
      ...
      <div className="mt-24 border-t border-slate-100 pt-16">
        <div className="flex items-center justify-between mb-12">
           <h2 className="text-3xl font-black text-slate-900">Avis Clients ({reviews.length})</h2>
           <div className="flex items-center gap-2">
              <span className="text-4xl font-black text-slate-900">4.5</span>
              <div className="flex flex-col">
                 <StarRating rating={4.5} readonly size={16} />
                 <span className="text-sm font-medium text-slate-400">Note moyenne</span>
              </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <div className="bg-slate-50 p-8 rounded-[32px] sticky top-24">
                {user ? (
                  <form onSubmit={handleSubmitReview} className="space-y-6">
                    <div>
                      <h3 className="text-xl font-bold text-slate-900 mb-2">Laissez votre avis</h3>
                      <p className="text-slate-500 text-sm">Votre expérience compte pour nous.</p>
                    </div>
                    
                    <div className="space-y-2">
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400">Note</label>
                    ...
                    <div className="space-y-2">
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400">Commentaire</label>
                      <textarea
                        value={comment}
                  ...
                  <div className="text-center space-y-6 py-4">
                     ...
                     <div>
                        <h3 className="text-xl font-bold text-slate-900 mb-2">Connectez-vous</h3>
                        <p className="text-slate-500 text-sm">Pour partager votre expérience et donner votre avis sur ce plat.</p>
                     </div>
                     <Link to="/auth">
           ...
           <div className="lg:col-span-2 space-y-8">
              {reviewsLoading ? (
                ...
                reviews.map(review => (
                  <div key={review.id} className="bg-white p-6 md:p-8 rounded-[32px] border border-slate-100 transition-all hover:shadow-lg hover:shadow-slate-200/50 hover:border-slate-200">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 text-lg">{review.user?.name || 'Anonyme'}</div>
