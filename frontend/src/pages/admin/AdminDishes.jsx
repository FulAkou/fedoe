...
const AdminDishes = () => {
  const { dishes, isLoading, error, fetchDishes, deleteDish, createDish, updateDish } = useDishStore();
  ...
  if (isLoading && dishes.length === 0) return <div className="flex justify-center items-center h-64"><Loader size="large" /></div>;
  ...
  return (
    <div className="p-6 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
       <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-gray-100 uppercase tracking-tight">Gestion du Menu</h2>
          <p className="text-slate-500 text-sm font-medium">Ajoutez, modifiez ou supprimez les plats du festival.</p>
        </div>
        <button 
      ...
      <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {dishes.map(dish => (
           <div key={dish.id} className="group bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-4 rounded-2xl flex flex-col gap-4 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300">
              ...
              <div className="flex-1 space-y-2">
                 <h3 className="font-black text-slate-900 dark:text-gray-100 text-lg">{dish.name}</h3>
                 <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 font-medium">{dish.description}</p>
              </div>
      ...
      >
        <form onSubmit={handleSave} className="space-y-6 py-2">
          ...
          <div className="flex gap-4 pt-6">
            <Button type="button" variant="outline" onClick={() => setIsModalOpen(false)} className="flex-1 rounded-xl h-12">
              Annuler
            </Button>
            <Button type="submit" className="flex-1 rounded-xl h-12 font-black" isLoading={isLoading}>
